...
must_haves:
  truths:
    ...
    - "OAuth session tokens are stored securely via Electron SafeStorage (existing secureStorage adapter)"
  artifacts:
    ...
    - path: "frontend/src/lib/supabase.ts"
      provides: "Supabase client with flowType pkce and secureStorage adapter"
      contains: "flowType"
  ...
  key_links:
    ...
    - from: "frontend/src/lib/supabase.ts"
      ...
      via: "storage: secureStorage passes OAuth tokens through Electron SafeStorage"
...
<task type="auto">
  <name>Task 2: Frontend OAuth store methods + callback listener + type definitions + secure storage verification</name>
...
- Keep existing `storage: secureStorage` — this is what ensures OAuth tokens are stored securely via Electron SafeStorage. Do NOT change or remove it.
...
**Secure storage verification:** After implementing the above, confirm that `supabase.ts` still has `storage: secureStorage` in the auth config. The existing Phase 4 secure storage adapter (`frontend/src/lib/secureStorage.ts`) already routes all Supabase token storage through Electron's SafeStorage IPC (`window.electron.secureStore.get/set/remove`). OAuth tokens from `exchangeCodeForSession` are stored by Supabase internally using this same adapter — no additional work needed. Verify by grepping: `grep -n "storage:" frontend/src/lib/supabase.ts` should show `storage: secureStorage`.

  </action>
  <verify>
Run `cd /Users/damifiance/workspace/AI-paper-reader/frontend && npx tsc --noEmit 2>&1 | head -30` to check for TypeScript errors. Verify authStore has `signInWithOAuth` method and `exchangeCodeForSession` call. Verify supabase.ts still has `storage: secureStorage` with `grep -n "storage:" frontend/src/lib/supabase.ts`.
  </verify>
  <done>
authStore.ts has signInWithOAuth method supporting google and github providers. In Electron mode, generates OAuth URL with skipBrowserRedirect, opens in external browser via IPC. OAuth callback listener in initialize() receives auth code from main process and exchanges for session via supabase.auth.exchangeCodeForSession(). supabase.ts has flowType: 'pkce' and retains storage: secureStorage (confirming OAuth tokens flow through Electron SafeStorage). electron.d.ts has types for startOAuth, onOAuthCallback, removeOAuthCallback.
  </done>
</task>
...
7. supabase.ts retains `storage: secureStorage` — OAuth tokens stored via Electron SafeStorage
</verification>
